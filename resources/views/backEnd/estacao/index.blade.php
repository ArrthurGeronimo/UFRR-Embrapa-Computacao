...
@section('content')
    ...
    <div class="table table-responsive">
        <table class="table table-bordered table-striped table-hover" id="tblestacao">
            <thead>
                <tr>
                    <th>ID</th><th>Talhao Id</th><th>Fazenda Id</th><th>Codigo</th><th>Actions</th>
                </tr>
            </thead>
            <tbody>
            @foreach($estacao as $item)
                <tr>
                    <td>{{ $item->id }}</td>
                    <td><a href="{{ url('estacao', $item->id) }}">{{ $item->talhao_id }}</a></td><td>{{ $item->fazenda_id }}</td><td>{{ $item->codigo }}</td>
                    <td>
