...
@section('content')
    ...
    <div class="table table-responsive">
        <table class="table table-bordered table-striped table-hover" id="tbloperacao">
            <thead>
                <tr>
                    <th>ID</th><th>Servico Id</th><th>Piquete Id</th><th>Insumo Id</th><th>Actions</th>
                </tr>
            </thead>
            <tbody>
            @foreach($operacao as $item)
                <tr>
                    <td>{{ $item->id }}</td>
                    <td><a href="{{ url('operacao', $item->id) }}">{{ $item->servico_id }}</a></td><td>{{ $item->piquete_id }}</td><td>{{ $item->insumo_id }}</td>
                    <td>
