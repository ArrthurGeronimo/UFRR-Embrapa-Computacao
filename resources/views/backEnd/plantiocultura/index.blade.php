...
@section('content')
    ...
    <div class="table table-responsive">
        <table class="table table-bordered table-striped table-hover" id="tblplantiocultura">
            <thead>
                <tr>
                    <th>ID</th><th>Piquete Id</th><th>Amostra Id</th><th>Data Amostra</th><th>Actions</th>
                </tr>
            </thead>
            <tbody>
            @foreach($plantiocultura as $item)
                <tr>
                    <td>{{ $item->id }}</td>
                    <td><a href="{{ url('plantiocultura', $item->id) }}">{{ $item->piquete_id }}</a></td><td>{{ $item->amostra_id }}</td><td>{{ $item->data_amostra }}</td>
                    <td>
